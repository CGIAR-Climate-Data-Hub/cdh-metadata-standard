...
- **`themes` (encoder output only)** — not authored. The encoder produces a
  themes block at serialization time from `cdh.domain`, `cdh.commodities`,
  `cdh.climate.hazards` (against `vocab/*.json`), and any linked-keyword
  entries.
...
- **Rules:**
  ...
  - Do not link entries to the `https://cgiar.org/cdh/vocab/*` schemes — those
    are reserved for encoder expansion from `cdh.domain`, `cdh.commodities`, and
    `cdh.climate.hazards`.
...
- `cdh.commodities` → scheme `https://cgiar.org/cdh/vocab/commodity`, populated
  with AGROVOC URIs via `vocab/commodity.json`;
- `cdh.climate.hazards` → scheme `https://cgiar.org/cdh/vocab/hazard`, populated
