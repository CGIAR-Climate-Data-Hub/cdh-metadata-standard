...
and `standard.yaml`) is encoded as an **OGC API Records** record using the
**recordJSON** schema
...
For most OGC Records resources, structured field metadata is not needed. When
it is (e.g., a tabular dataset surfaced via OGC Records rather than STAC):
...
otherwise match the STAC profile exactly. OGC Records has no equivalent of
STAC `summaries`; faceted values that would be in STAC summaries appear as
...
For OGC Records, file-level metadata lives on the link, not as top-level
record metadata:
...
The required CDH `data[]` entries map to `links[rel=enclosure]` for
downloadable files, or `links[rel=service]` for service endpoints. If the
resource is a landing page, code repository, dashboard, or model, use
`rel=about`, `rel=code` (`processing-expression` for workflow code), or the
most appropriate relation from §5.1.
...
2. The first step's `code.url` maps to `links[rel=processing-expression]`
   on the record. Include `cgiar-cdh:code_version` as a link extra field.
